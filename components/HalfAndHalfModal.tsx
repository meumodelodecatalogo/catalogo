...
export default function PizzaSelectionModal({ isOpen, onClose, firstFlavor, allProducts }: PizzaSelectionModalProps) {
  ...
  const handleConfirm = () => {
    ...
    try {
      if (pizzaType === "inteira") {
        const finalPrice = firstFlavor.price * selectedSize.multiplier;
        addToCart(firstFlavor, selectedSize.name, finalPrice);
        onClose();
      } else if (pizzaType === "meio-a-meio" && flavor1 && flavor2) {
        const basePrice = Math.max(flavor1.price, flavor2.price);
        const finalPrice = basePrice * selectedSize.multiplier;
  ...
  const currentPrice = selectedSize 
    ? (pizzaType === "meio-a-meio" 
        ? (flavor1 && flavor2 ? Math.max(flavor1.price, flavor2.price) : (flavor1 ? flavor1.price : firstFlavor.price))
        : firstFlavor.price) * selectedSize.multiplier
    : firstFlavor.price;
                      ...
                      {PIZZARIA_CONFIG.pizzaSizes.map((size) => (
                        ...
                        >
                          ...
                          <span className="text-sm font-bold text-gray-400">
                            R$ {(firstFlavor.price * size.multiplier).toFixed(2).replace(".", ",")}
                          </span>
                        </button>
                      ))}
                    </div>
